 ...
 *
 * $Id: Database_Core.php 4342 2009-05-08 16:56:01Z jheathco $
 *
 * @package    Core
 * @author     Kohana Team
 ...
 */
require_once("Third/kohana/libraries/drivers/Database.php");
require_once("Third/kohana/libraries/drivers/Database/Mysql.php");
require_once("Third/kohana/libraries/drivers/Database/Mysqli.php");

class Database_Core {

	// Database_Core instances
	...
	/**
	 * Returns a singleton instance of Database_Core.
	 *
	 * @param   mixed   configuration array or DSN
	 * @return  Database_Core_Core
	 */
	public static function & instance($name = 'default', $config = NULL)
	{
		if ( ! isset(Database_Core::$instances[$name]))
		{
			// Create a new instance
			Database_Core::$instances[$name] = new Database_Core($config === NULL ? $name : $config);
		}

		return Database_Core::$instances[$name];
	}

	/**
	 * Returns the name of a given database instance.
	 *
	 * @param   Database_Core  instance of Database_Core
	 * @return  string
	 */
	public static function instance_name(Database_Core $db)
	{
		return array_search($db, Database_Core::$instances, TRUE);
	}

	/**
	 * Sets up the database configuration, loads the Database_Core_Driver.
	 *
	 * @throws  Kohana_Database_Core_Exception
	 */
	public function __construct($config = array())
	{
		...
		elseif (is_string($config))
		{
			...
			else
			{
				...
				if (($config = Kohana::config('database.'.$config)) === NULL)
					throw new Kohana_Database_Core_Exception('database.undefined_group', $name);
			}
		}
		...
		if (is_string($this->config['connection']))
		{
			...
			if (strpos($this->config['connection'], '://') === FALSE)
				throw new Kohana_Database_Core_Exception('database.invalid_dsn', $this->config['connection']);
		...
		// Set driver name
		$driver = 'Database_Core_'.ucfirst($this->config['connection']['type']).'_Driver';

		// Load the driver
		//if ( ! Kohana::auto_load($driver))
		//	throw new Kohana_Database_Core_Exception('core.driver_not_found', $this->config['connection']['type'], get_class($this));

		// Initialize the driver
        
		$this->driver = new Database_Mysqli_Driver($this->config);
	...
	public function connect()
	{
		...
		if ( ! is_resource($this->link) AND ! is_object($this->link))
		{
			...
			if ( ! is_resource($this->link) AND ! is_object($this->link))
				throw new Kohana_Database_Core_Exception('database.connection', $this->driver->show_error());
	...
	public function query($sql = '')
	{
		...
		{
			// Benchmark the query
			Database_Core::$benchmarks[] = array('query' => $sql, 'time' => $stop - $start, 'rows' => count($result));
		}
	 ...
	 * @return  Database_Core_Core  This Database_Core object.
	 */
	...
	public function get($table = '', $limit = NULL, $offset = NULL)
	{
		...
		//file_put_contents('/tmp/log.log', $sql."\n", FILE_APPEND);
	 ...
	 * @return  Database_Core_Result  Query result
	 */
	public function insert($table = '', $set = NULL)
	{
		...
		if ($this->set == NULL)
			throw new Kohana_Database_Core_Exception('database.must_use_set');

		if ($table == '')
		{
			if ( ! isset($this->from[0]))
				throw new Kohana_Database_Core_Exception('database.must_use_table');
	...
	public function update($table = '', $set = NULL, $where = NULL)
	{
		...
		//file_put_contents('/tmp/yunjin.update.log', $sql."\n", FILE_APPEND);
	...
	public function delete($table = '', $where = NULL)
	{
		...
		if (count($this->where) < 1)
			throw new Kohana_Database_Core_Exception('database.must_use_where');
	 ...
	 * executed
	 *
	 * @return Database_Core_Core This Databaes object
	 */
...
/**
 * Sets the code for a Database_Core exception.
 */
class Kohana_Database_Exception extends Kohana_Exception {

	protected $code = 11211111;

} // End Kohana Database_Core Exception
